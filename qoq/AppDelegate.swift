...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        hotKeys = HotKeyManager(
            ...
            extractText: shortcuts.extractText,
            translateSelection: { [weak self] in self?.translateSelection() },
            captureScreen: { [weak self] in self?.captureScreen() },
            extractScreenText: { [weak self] in self?.extractScreenText() }
        )
        shortcuts.onChange = { [weak self] selection, capture, extractText in
            self?.hotKeys?.update(selection: selection, capture: capture, extractText: extractText) ?? false
        }
